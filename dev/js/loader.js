exports.sceneLoader = function(manager, scene, THREE){
  ...
  this.createScene = function(){
    ...
    function loadHdris(item, i, callback){
    ...
    function buildMaterials(materialName, i, callback){

      let material = scope.json.materials[materialName]
    ...
    function loadMeshes(item, i, callback){
      if(!meshes[item.name]){
        scope.jsonloader.load("./3D/"+scope.name+"/models/"+item.name+".json", function(mesh){
    ...
    function buildStaticMesh(item){
      let _mesh = meshes[item.name];

      if(item.size){
        _mesh.scale(item.size)
      }

      if (item.rot){
        _mesh.rotateX(item.rot[0]*Math.PI/180)
        _mesh.rotateY(item.rot[1]*Math.PI/180)
        _mesh.rotateZ(item.rot[2]*Math.PI/180)
      }

      if (item.pos){
        _mesh.translate(item.pos[0], item.pos[1], item.pos[2])
      }

      materialArray.push(materials[item.name])
      staticmeshes.push({"name": item.name, "mesh": _mesh, "materialIndex": materialArray.length-1})

    }

    function buildDynamicMesh(item){

      let mesh = new THREE.Mesh(new THREE.BufferGeometry().fromGeometry(meshes[item.name]), materials[item.material]);
    ...
    function buildObjects(item, i, callback){

      //Check for missing materials
      if (!materials[item.material]) {
        console.error("Missing material for: "+item.name)
      }

      if (item.promote) {
        buildDynamicMesh(item)
        callback();
      }else{
        buildStaticMesh(item)
        callback();
      }
    ...
    function mergeStaticMeshes(item, i, callback){

      if(!staticMesh){staticMesh = new THREE.Geometry()}
    ...
    scope.looper(scope.json.hdris, loadHdris, function(){
      //Load all materials
      let _materials = Object.keys(scope.json.materials)
      scope.looper(_materials, buildMaterials, function(){

        //Load all the meshes
        scope.looper(scope.json.objects, loadMeshes, function(){

          //Combine all the meshes and materials into objects
          scope.looper(scope.json.objects, buildObjects, function(){

            //Merge the static meshes into one
            if(staticmeshes.length>0){
              scope.looper(staticmeshes, mergeStaticMeshes, function(){
